...
const fetchLanguageRatio = async (): Promise<Language[]> => {
  ...
  codeMetadata.forEach(({ extension, bytes }) => {
    const langaugeAndColor = convertExtensionToLanguageIfAllowed(extension);
...
const getMarkdownCodeMetadata = async (
  ...
  return [codeDirectivesMetadata, inlinedCodeExtensionAndSize].flat();
};
...
const convertExtensionToLanguageIfAllowed = (extension: string) => {
